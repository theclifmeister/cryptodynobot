...
confirmAddress = ctx => {
  ctx.replyWithHTML(`Your withdrawal address is <b>${step.withdraw_address}</b> is this correct?`, Markup.inlineKeyboard([Markup.callbackButton('Yes', 'confirm-address-yes'), Markup.callbackButton('No', 'confirm-address-no')]).extra());
};
...
scene.enter(async ctx => {
  ...
  if (step.withdraw_address) {
    ...
    ctx.replyWithHTML('Please respond to us with a valid <b>ETH</b> withdrawal address.');
  }
});
...
scene.action('confirm-value-yes', async ctx => {
  return ctx.replyWithHTML(`Great! We will send <b>${step.withdraw_value}</b> ETH to <b>${step.withdraw_address}</b> you will recieve a transaction id by e-mail.`);
});

scene.action('confirm-value-no', async ctx => {
  return ctx.replyWithHTML('How much <b>ETH</b> do you want to withdraw?');
});
